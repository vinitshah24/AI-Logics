...
def truth_table_rows(variables):
    ...
    def add_P(v): return [dict([(P, v)] + list(r.items())) for r in R]
...
class Proposition:
    ...
    def print_truth_table(self):
        ...
        header = "{0}  #  {1: ^{2}}".format(
            "  ".join("{0: ^5}".format(v) for v in vars), formula_header, formula_len)
        print(header)
        print("#"*len(header))
        for r in rows:
            var_cols = "  ".join("{0: ^{1}}".format(
                str(r[v]), max(5, len(v))) for v in vars)
            result_col = "{0: ^{1}}".format(
                str(self.evaluate(**r)), formula_len)
    ...
    def is_equivalent(self, other):
        ...
        return all(self.evaluate(**r) == other.evaluate(**r)
                   for r in truth_table_rows(self.variables() | other.variables()))
...
class ArgumentForm:
    ...
    def __str__(self):
        return ((", ".join(str(c) for c in self.premises) + ", ") if self.premises else "") + \
    ...
    def print_truth_table(self):
        ...
        print("{0}  #  {1: ^{2}}  #  {3: ^{4}}".format(" "*len(vars_header),
              "premises", len(premises_header), "conclusion", len(conclusion_string)))
        header = "{0}  #  {1: ^8}  #  {2}".format(
            vars_header, premises_header, conclusion_string)
        print(header)
        print("#"*len(header))
        for r in rows:
            ...
            var_cols = "  ".join("{0: ^{1}}".format(
                str(r[v]), len(k)) for (k, v) in zip(var_strings, vars))
            premise_cols = "   ".join("{0: ^{1}}".format(
                str(v)+star, len(k)) for (k, v) in zip(premise_strings, premise_values))
            conclusion_col = "{0: ^{1}}".format(
                str(conclusion_value)+star, len(conclusion_string))
            print("{0}  #  {1: ^8}  #  {2}".format(
                var_cols, premise_cols, conclusion_col))
        print()

    def is_valid(self):
        vars = (frozenset.union(*[c.variables() for c in self.premises])
                if self.premises else frozenset()) | self.conclusion.variables()
        return all(self.conclusion.evaluate(**r) for r in truth_table_rows(vars)
                   if all(c.evaluate(**r) for c in self.premises))
